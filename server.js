...
async function loadDB() {
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    try {
      const { blobs } = await list({ prefix: 'icons.json' });
      const jsonBlob = blobs.find(b => b.pathname === 'icons.json');
      if (jsonBlob) {
        const res = await fetch(jsonBlob.url);
        iconsDB = await res.json();
        return;
      }
    } catch(err) {
      console.error("Vercel Blob load error (it may be empty):", err);
    }
  } else {
    // Local fallback
  ...
  iconsDB = DEFAULT_ICONS.map(icon => ({ ...icon, created_at: new Date().toISOString() }));
  await saveDB();

  for (const icon of iconsDB) {
    await saveSVG(icon.id, icon.svg_code);
  }
}

async function saveDB() {
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    await put('icons.json', JSON.stringify(iconsDB, null, 2), { access: 'public', addRandomSuffix: false });
  } else {
    fs.writeFileSync(DB_PATH, JSON.stringify(iconsDB, null, 2));
  }
}

async function saveSVG(id, svg_code, localFilePath = null) {
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    await put(`icons/${id}.svg`, svg_code, { access: 'public', addRandomSuffix: false, contentType: 'image/svg+xml' });
    if (localFilePath && fs.existsSync(localFilePath)) {
      fs.unlinkSync(localFilePath);
    }
  } else {
    fs.writeFileSync(path.join(ICONS_DIR, `${id}.svg`), svg_code);
    if (localFilePath && fs.existsSync(localFilePath)) {
      fs.unlinkSync(localFilePath);
    }
  }
...
async function deleteSVG(id) {
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    const { blobs } = await list({ prefix: `icons/${id}.svg` });
    if (blobs.length > 0) {
...
app.post("/upload", upload.single("icon"), async (req, res) => {
  try {
    ...
    await saveSVG(newIcon.id, svgCode, req.file.path);
